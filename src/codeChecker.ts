...
import { CodeType, toJsString, toTypedString } from './helpers';
...
type TestCode =
  ...
  | {
      ...
      name?: string;
      /**
       * Type name to test against (assigns to variable)
       * @example
       * ```ts
       * type TypedObject = { a: string; b: number; c: (a: number, b: number) => number };
       *
       * test({
       *  code: { a: 'test', b: 1, c: (a, b) => a + b },
       *  name: 'typedObject',
       *  typeName: 'TypedObject'
       * });
       * // returns string => `const typedObject = { a: 'test', b: 1, c: (a, b) => a + b }`;
       * ```
       */
      typeName?: string;
    };
...
export class CodeChecker<TypeNames extends string> {
  ...
  private _test(testCode: TestCode, typeName?: TypeNames) {
    if (typeof testCode !== 'string') {
      const { code, name, typeName } = testCode;

      testCode =
        name && typeName
          ? toTypedString(code, name, typeName)
          : toJsString(code);
    }
  ...
  public test(testCode: TestCode, typeName?: TypeNames) {
    const { valid } = this._test(testCode, typeName);

    return valid;
  }

  public assert(testCode: TestCode, typeName?: TypeNames) {
  ...
  public static toJsString = toJsString;
  public static toTypedString = toTypedString;

  public toJsString = toJsString;
  public toTypedString = toTypedString;
}
